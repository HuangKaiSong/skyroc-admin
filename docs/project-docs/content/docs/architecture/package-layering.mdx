...
底层包**不允许**反向依赖上层（比如 `@skyroc/service` 不能 import antd）。当底层确实需要上层能力时，用**适配器接口**：
...
应用层实现这个接口，把 antd 的 message、TanStack Router 的导航注入进去。详见 [`@skyroc/service`](/docs/core/service) 与 [Admin 请求层](/docs/admin-app/service)。
