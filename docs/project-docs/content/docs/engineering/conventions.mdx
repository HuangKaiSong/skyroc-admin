...
其他都禁止。详见 [Vercel React 最佳实践 §5.3](/docs/architecture/package-layering)。
...
提交工具：`pnpm commit`（在 `apps/admin` 等包内），由 `sa git-commit` 提供交互式表单。详见 [Git 提交与发版](/docs/engineering/git-commit)。
