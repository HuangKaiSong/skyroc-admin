...
| `apps/admin/src/service/api/route/types.d.ts` | 定义后端路由结构，包含 `path`、`layout`、`permissions`、`children`。 |
| `apps/admin/src/bootstrap.tsx` | 通过 `loadDynamicRoutes` 把 `queryMenusOptions()` 注入布局运行时。 |
...
| 登录守卫和权限 | [权限守卫](./routing/guards-and-permissions) |
