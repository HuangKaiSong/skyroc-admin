...
动态路由模式不是“后端临时注册 React 页面组件”。后台返回的菜单 path 仍然必须能落到前端已经存在的 TanStack Route，否则用户点击后只会进入前端 404 或找不到页面。
...
| `dynamic` | 后端 `fetchGetBackendRoutes()` 返回的路由树 | 后端路由节点上的 `permissions`，再结合前端用户角色 | 前端 `apps/admin/src/pages` |

动态模式下，`bootstrap.tsx` 提供的 `loadDynamicRoutes` 会调用：

```ts
queryClient.ensureQueryData(queryMenusOptions())
```

接口返回类型在 `apps/admin/src/service/api/route/types.d.ts` 中定义：
...
`BackendRoute` 继承 `Router.Meta`，所以后端动态菜单节点和前端 `staticData` 使用同一套元信息字段，例如 `title`、`i18nKey`、`menu`、`permissions`、`href` 和 `url`。
...
| 主题 | 继续查看 |
| --- | --- |
| 路由标题、菜单、图标、隐藏页、外链 | `routing/route-meta.mdx` |
| 登录拦截、403、静态/动态权限 | `routing/guards-and-permissions.mdx` |
| 应用启动时如何注入 route tree 和菜单配置 | `architecture/bootstrap.mdx` |
| 目录职责和 app/package 边界 | `getting-started/project-structure.mdx` |
