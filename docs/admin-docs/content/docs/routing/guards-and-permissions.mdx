...
| `setState({ initialized: true })` | 标记 auth 初始化完成，避免每次路由切换都重复初始化。 |
...
| 已登录访问 `/login` 没有自动跳走 | `context.isLoggedIn` 和 `context.isAuthInitialized` 是否正确注入到 RouterProvider。 |
...
| 在页面组件里再请求用户信息更安全 | 当前用户初始化已经在 root/admin guard 链路完成；页面里重复初始化会制造状态竞争。 |

## 相关位置

| 主题 | 继续查看 |
| --- | --- |
| 页面文件、路由组和 route tree | `routing/overview.mdx` |
| `staticData`、`permissions`、`href`、`activeMenu` | `routing/route-meta.mdx` |
| 启动时注入 `setupAdminLayouts` | `architecture/bootstrap.mdx` |
| 登录页和退出登录页面 | `features/auth-login.mdx` |
