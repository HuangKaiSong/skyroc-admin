...
// Types
export type { ThemeConfig, ThemeColors, ThemeMode, ThemeToken, ColorPalette } from './types';
```
...
export const themeConfigAtom = atomWithStorage<ThemeConfig>('theme-config', defaultThemeSettings);
...
export function createThemeToken(colors: ThemeColors, isDarkMode: boolean): ThemeToken {
...
function App() {
  ...
  return <ThemeProvider theme={tokens}>{/* 子组件通过 context 访问主题 */}</ThemeProvider>;
}
```
...
function ThemeToggle() {
  ...
  return <Button onClick={() => setMode(isDarkMode ? 'light' : 'dark')}>{isDarkMode ? '🌙 Dark' : '☀️ Light'}</Button>;
}
```
...
function ColorPicker() {
  ...
  return <input type="color" value={colors.primary} onChange={e => setPrimaryColor(e.target.value)} />;
}
```
