...
// Types
export type { LangType, LangConfig, LangOption, I18nConfig } from './types';
```
...
function LanguageSelector() {
  ...
  return (
    <select value={locale} onChange={e => changeLocale(e.target.value as LangType)}>
...
function LanguageToggle() {
  ...
  return <button onClick={toggleLang}>{locale === 'zh-CN' ? '中文' : 'English'}</button>;
}
```
