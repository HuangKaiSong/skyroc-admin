...
- `Dayjs` 初始化：注册 `localeData` 插件，并把宿主应用当前语言同步给 Dayjs。
...
@skyroc/web-admin-runtime
  ...
  ├─ setupDayjs()                  Dayjs localeData 与语言同步
...
`setupI18n()` 仍然单独保留。它是异步初始化，并且通常要在 React render 前完成。runtime 包不应该把这个时序吞进一个看似通用的 helper。
...
setupDayjs({
  withLocaleData: true,
  syncLocale() {
    // set dayjs locale from host app
  }
});
```
...
| `syncLocale`     | `undefined` | 宿主应用同步当前语言到 Dayjs 的回调。 |

`setupDayjs` 只注册 Dayjs 插件和调用同步回调。具体支持哪些语言、语言 key 如何映射到 Dayjs locale，仍然由宿主应用控制。
...
function setDayjsLocale(lang: 'zh-CN' | 'en-US' = 'zh-CN') {
  const localeMap = {
    'zh-CN': 'zh-cn',
    'en-US': 'en'
  };
...
- `setupAdminPlugins()` 中的 Dayjs locale 同步要早于 i18n 初始语言变更。
...
`setupI18n()` 是异步流程，会加载语言资源、初始化 i18next、写语言状态，并触发第三方语言同步。它属于应用启动编排，不能被一个同步 runtime helper 隐藏。
...
`@skyroc/web-admin-runtime` 提供通用能力，`apps/admin/src/plugins/index.ts` 负责把本应用的 env、router、通知 UI、语言同步和 NProgress 实例回传组合成具体配置。
