...
const CountDownDemo = () => {
    ...
    return (
      <View className="flex-row items-center gap-1">
        <View className="rounded bg-primary px-1.5 py-0.5">
          <Text className="text-sm font-bold text-primary-foreground">{String(current.hours).padStart(2, '0')}</Text>
        </View>
        <Text className="text-sm font-bold text-foreground">:</Text>
        <View className="rounded bg-primary px-1.5 py-0.5">
          <Text className="text-sm font-bold text-primary-foreground">{String(current.minutes).padStart(2, '0')}</Text>
        </View>
        <Text className="text-sm font-bold text-foreground">:</Text>
        <View className="rounded bg-primary px-1.5 py-0.5">
          <Text className="text-sm font-bold text-primary-foreground">{String(current.seconds).padStart(2, '0')}</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <ScrollView className="flex-1 bg-muted" contentContainerClassName="pb-20" showsVerticalScrollIndicator={false}>
      ...
      <View className="bg-background px-4 py-4">
        <CountDown time={30 * 60 * 60 * 1000}>{renderCustom}</CountDown>
      </View>

      {/* Manual Control */}
      <Text className="px-4 py-3 text-sm text-muted-foreground">Manual Control</Text>
      <View className="bg-background px-4 py-4">
        <CountDown ref={countDownRef} autoStart={false} format="ss:SSS" millisecond time={3 * 1000} />
        <View className="mt-3 flex-row gap-2">
          {manualStatus === 'paused' ? (
            <Button size="sm" onPress={handleStart}>
              Start
            </Button>
          ) : (
            <Button size="sm" onPress={handlePause}>
              Pause
            </Button>
          )}
