...
const TagDemo = () => {
  ...
  return (
    <ScrollView className="flex-1 bg-muted" contentContainerClassName="p-6 pb-20" showsVerticalScrollIndicator={false}>
      ...
      <View className="mb-8 flex-row flex-wrap items-center gap-3">
        {COLORS.map(c => (
          <Tag key={c} color={c} variant="tonal">
            {c}
          </Tag>
        ))}
      </View>
      ...
      <View className="mb-8 flex-row flex-wrap items-center gap-3">
        {COLORS.map(c => (
          <Tag key={c} color={c} variant="outline">
            {c}
          </Tag>
        ))}
      </View>
      ...
      <View className="mb-8 flex-row flex-wrap items-center gap-3">
        {COLORS.map(c => (
          <Tag key={c} color={c} variant="ghost">
            {c}
          </Tag>
        ))}
      </View>
      ...
      <View className="mb-8 flex-row flex-wrap items-center gap-3">
        {visible ? (
          <Tag closeable onClose={() => setVisible(false)}>
            Click to close
          </Tag>
        ) : (
          <Text className="text-sm text-muted-foreground">Tag closed</Text>
        )}
        <Tag closeable color="success" onClose={() => Alert.alert('Close')}>
          Success
        </Tag>
        <Tag closeable color="warning" variant="tonal" onClose={() => Alert.alert('Close')}>
          Warning
        </Tag>
      </View>
      ...
      <View className="mb-8 flex-row flex-wrap items-center gap-3">
        <Tag color="primary" shape="pill" size="sm" variant="tonal">
          Small Pill
        </Tag>
        <Tag closeable color="destructive" shape="pill" variant="outline" onClose={() => Alert.alert('Close')}>
          Closeable Pill
        </Tag>
        <Tag color="success" shape="mark" size="lg">
          Large Mark
        </Tag>
        <Tag color="info" shape="mark" variant="tonal">
          Tonal Mark
        </Tag>
      </View>
    </ScrollView>
