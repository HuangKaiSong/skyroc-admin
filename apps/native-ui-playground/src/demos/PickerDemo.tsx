...
const PickerDemo = () => {
  ...
  return (
    <ScrollView className="flex-1 bg-muted" contentContainerClassName="pb-20" showsVerticalScrollIndicator={false}>
      ...
      <PickerView columns={FRUITS} defaultValue={['orange']} haptic showToolbar={false} />

      {/* Multiple Columns */}
      <Text className="px-4 py-3 text-sm text-muted-foreground">Multiple Columns</Text>
      <PickerView columns={[YEARS, MONTHS, DAYS]} defaultValue={['2026', '2', '21']} title="Select Date" />

      {/* Cascade */}
      <Text className="px-4 py-3 text-sm text-muted-foreground">Cascade</Text>
      <PickerView columns={CASCADE_DATA} defaultValue={['zhejiang', 'hangzhou', 'xihu']} title="Select Region" />
      ...
      <View className="bg-background px-4 py-3">
        <View className="flex-row items-center gap-3">
          ...
          <Text className="text-sm text-muted-foreground">Selected: {sheetValue.join(', ') || 'None'}</Text>
        </View>
      ...
      <View className="bg-background px-4 py-3">
        <Button size="sm" onPress={loadAsyncData}>
          Reload Data
        </Button>
      </View>
      <PickerView
