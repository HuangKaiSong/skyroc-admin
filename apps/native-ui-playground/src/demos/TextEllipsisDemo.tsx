...
const TextEllipsisDemo = () => {
  ...
  return (
    <ScrollView className="flex-1 bg-muted" contentContainerClassName="pb-20" showsVerticalScrollIndicator={false}>
      ...
      <View className="bg-background px-4 py-4">
        <TextEllipsis collapseText="收起" content={LONG_TEXT} expandText="展开" rows={2} />
      </View>

      {/* Custom Dots */}
      <Text className="px-4 py-3 text-sm text-muted-foreground">Custom Dots</Text>
      <View className="bg-background px-4 py-4">
        <TextEllipsis content={LONG_TEXT} dots=" ···" expandText="展开" rows={2} />
      </View>

      {/* Programmatic Control */}
      <Text className="px-4 py-3 text-sm text-muted-foreground">Programmatic Control</Text>
      <View className="bg-background px-4 py-4">
        <TextEllipsis ref={ellipsisRef} content={LONG_TEXT} rows={2} />
        <View className="mt-3 flex-row gap-2">
          <Button size="sm" onPress={() => ellipsisRef.current?.toggle(true)}>
            Expand
          </Button>
          <Button size="sm" variant="outline" onPress={() => ellipsisRef.current?.toggle(false)}>
            Collapse
          </Button>
        </View>
      </View>
